...
def run_one_simulation(seed, culture, flask, init_R, inher_R, Ta, alpha, t_grow, rep, cycle, mutation_function):
    ...
    # mutation
    genotype_n_sep_mut = generate_mutants(flask, copy.deepcopy(genotype_n_sep1), len(flask[0].genotypes),
    ...
    for s, species in enumerate(flask):
        for i, genotype in enumerate(species.genotypes):
            final_sub.append((seed, culture, rep, cycle, 2, species.name, genotype.name,
                              genotype_n_sep2[2 * i + (0, 2 * nE)[s]], genotype_n_sep2[2 * i + (1, 2 * nE + 1)[s]],
                              genotype_n_unsep2[i + (0, nE)[s]], genotype.lag, Ta))
...
def run(seed, culture, reps, mu, cycles, init_R, init_n, init_lag, Ta, alpha, t_grow, mutation_func_type,
    ...
    final_pd.to_csv(
        f'{culture}_seed{seed}_rep{reps}_mu{mu}_cycles{cycles}_init_R{init_R}_init_n{init_n}_init_lag{init_lag}_Ta{Ta}_alpha{alpha}_{mutation_func_type}{max_lag_change}.csv',
        index=False)
