...
# mets = [1]
lacs = [1, 5, 10, 20, 50, 100, 200, 500, 1000, 1500]
...
for lac in lacs:
    ...
    file_mono = f"C:\\Users\\untit\\harcombe\\data_hold\\hcb_sim_mono_{seed}_met{1000}_lac{lac}.csv"
    ...
    frac_file_mono = f"C:\\Users\\untit\\harcombe\\data_hold\\frac_hcb_sim_mono_{seed}_met{1000}_lac{lac}.csv"
    # frac_file_co = f"C:\\Users\\untit\\harcombe\\data_hold\\frac_hcb_sim_co_{seed}_met{met}_lac{lac}.csv"

    # compute per rep
    data1 = pd.read_csv(frac_file_mono, header=2, na_filter=False)
    ...
    for rep in reps:
        ...
        frac_rates.append(('Monoculture', rep, 1000, lac, slope_mono))
...
frac_rates_pd.to_csv(f'frac_rates_met{[1000]}_lac{lacs}.csv', index=False)