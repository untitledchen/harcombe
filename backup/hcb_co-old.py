...
def run_phase(odes, init_cond, t_interval, tau_lag, n, frid=False, phase=1):
    ...
    sol = odeint(odes, init_cond, t_interval,
                 args=(alpha, tau_lag, n, frid))  ## when alpha = -2, effective growth rate = -r
    para = [t_interval, tau_lag, n]

    plt.plot(t_interval, sol[:, 0], label='met', color='r')
    plt.plot(t_interval, sol[:, 1], label='ace', color='b')
    plt.plot(t_interval, sol[:, 2], label='lac', color='y')
...
def solplot(sols, paras):
    ...
    for s in range(len(sols)):
        ...
        for i in range(1, nE + 1):
            ...
            axs[0][s].plot(t_interval, np.log10(freqsE), color=(0, 0.8 - g * i, 0),
                           label=f'E. coli strain {i}, lag time = {str(tau_lag[0][i - 1])}')
        b = 0.8 / (1.5 * nS)
        for j in range(1, nS + 1):
            freqsS = (locals()[f'Sg{j}{s}'] + locals()[f'Sl{j}{s}']) / totals
            axs[0][s].plot(t_interval, np.log10(freqsS), color=(0, 0, 0.8 - b * j),
                           label=f'S. enterica strain {j}, lag time = {str(tau_lag[1][j - 1])}')
...
def regular(init_M, init_A, init_L, Ta, tau_lag=None, n=[1, 1], frid=False):
    ...
    init_cond = [init_M, init_A, init_L] + [0, 1 / (n[0] + n[1])] * (
                n[0] + n[1])  ## each strain for each species starts with growing population 0 and lagging population 1
    ...
    sol2, para2 = run_phase(odes, [1, 0, 1000] + list(sol1[-1, 3:]), t_interval2, tau_lag, n, frid=frid, phase=2)
...
def taus(init_M, init_A, init_L, Ta, n=[1, 1], taulim=12, plot=True):
    ...
    init_cond = [init_M, init_A, init_L] + [0, 1] * (
                n[0] + n[1])  ## each strain for each species starts with growing population 0 and lagging population 1
...
def threed(init_M, init_A, init_L, n=[1, 1], taulim=12, Talim=12,
           colors=None):  ## colors is a temporary fix because its 10:27pm
...
if inp == 's':
    print(
        'Running regular(10, 10, 10, 6, tau_lag=[[3, 6, 9], [6]], n=[3,1]). Initial methionine, initial acetate, initial lactose: 10; Ta: 6; tau_lag: 3,6,9 for E. coli and 6 for S. enterica; E. coli strains: 3; S. enterica strains: 1.')
    regular(1, 0, 1000, 6, tau_lag=[[3, 6, 9], [6]], n=[3, 1])
