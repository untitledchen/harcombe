...
class Genotype():
    ...
    def __str__(self):
        return "{'name': " + str(self.name) + ", 'n': " + str(self.n) + ", 'lag': " + str(
            self.lag) + ", 'ancestors': " + str(self.ancestors) + "}"
    ...
    for species, growing in enumerate(genotype_n_growing):  # [genotype_n_growing[0]] to exclude S
        ...
        if mutant_n != 0:
            ...
            for j, anc_i in enumerate(mutants):
                ...
                genotype_n_sep[2 * anc_i + (1, 2 * nE + 1)[
                    species]] -= 1  # tuple is for correct indices in genotype_n_sep, which is not separated by species

                if species == 0:
                    x = np.array(genotype_n_sep)
                    x = np.insert(x, (2 * nE, len(genotype_n_sep))[species], 1)
                    genotype_n_sep = list(x)
                elif species == 1:
                    genotype_n_sep.append(1)  ##
                genotype_n_sep.append(0)

                flask[species].add_genotype(Genotype(f"{('E', 'S')[species]}{genotype_ct + j}c{cycle}", n=1,
                                                     lag=max([0, ancestor.lag + mutation_function(species)]),
...
def run_one_simulation(seed, culture, flask, init_R, inher_R, Ta, alpha, t_grow, rep, cycle, mutation_function):
    ...
    if True: #cycle > 0:
        ...
        for s, species in enumerate(flask):
            for i, genotype in enumerate(species.genotypes):
                final_sub.append((seed, culture, rep, cycle, 1, species.name, genotype.name,
                                  genotype_n_sep1[2 * i + (0, 2 * nE)[s]], genotype_n_sep1[2 * i + (1, 2 * nE + 1)[s]],
                                  genotype_n_unsep1[i + (0, nE)[s]], genotype.lag, Ta))
    ...
    # phase 2
    sol2 = run_phase(alpha, init_cond, lags1, t_grow, 2)
    ...
    for s, species in enumerate(flask):
        for i, genotype in enumerate(species.genotypes):
            final_sub.append((seed, culture, rep, cycle, 2, species.name, genotype.name,
                              genotype_n_sep_mut[2 * i + (0, 2 * nE)[s]], genotype_n_sep_mut[2 * i + (1, 2 * nE + 1)[s]],
                              genotype_n_unsep_mut[i + (0, nE)[s]], genotype.lag, Ta))
...
# simulation
def run(seed, culture, reps, mu, cycles, init_R, init_n, init_lag, Ta, alpha, t_grow, mutation_func_type,
        max_lag_change):
    ...
    final = [
        ('seed', 'culture', 'rep', 'cycle', 'phase_end', 'species', 'genotype', 'nlag', 'ngrow', 'ntot', 'lag', 'Ta')]
    for rep in range(reps):
        ...
        if culture == "co":
            ...
            final.append(
                (seed, culture, rep, 0, 0, 'Salmonella enterica', 'S0c0', init_n[1], 0, init_n[1], init_lag[1], Ta))

        inher_R = (0, 0, 0)
        # run simulation
        for cycle in range(cycles):
            print(f"Cycle {cycle}")  #
            final_sub, inher_R = run_one_simulation(seed, culture, flask, init_R, inher_R, Ta, alpha, t_grow, rep,
                                                    cycle, mutation_func)
    ...
    final_pd.to_csv(
        f'{culture}_seed{seed}_rep{reps}_mu{mu}_cycles{cycles}_init_R{init_R}_init_n{init_n}_init_lag{init_lag}_Ta{Ta}_alpha{alpha}_{mutation_func_type}{max_lag_change}_postmut.csv',
        index=False)
...
run(seed, "co", 10, (0.001, 0.001), 10, (1, 2780, 0), (5, 5), (1, 1), 5, (3, 3), 42, "null", (1.1, 1.1))
# run(seed, "mono", 10, (0.001, 0.001), 10, (80, 2780, 0), (5, 5), (1, 1), 5, (3, 3), 42, "null", (1.1, 1.1))